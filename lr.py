...
lrs = [0.00025, 0.0005, 0.001, 0.0025, 0.005]
...
for _, lr in enumerate(lrs):
    scores[_], epsilons[_] = train(learning_rate=lr, name="lr"+str(lr))
    pd.Series(scores[_]).to_csv("scores_lr"+str(lr))
    pd.Series(epsilons[_]).to_csv("epsilons_lr"+str(lr))
...
lrs00025=ax.plot(pd.Series(scores[0]).rolling(window=100).mean(),'y',label='lr0.00025',alpha=1)
lrs0005=ax.plot(pd.Series(scores[1]).rolling(window=100).mean(),'b',label='lr0.0005',alpha=1)
lrs001=ax.plot(pd.Series(scores[2]).rolling(window=100).mean(),'r',label='lr0.001',alpha=1)
lrs0025=ax.plot(pd.Series(scores[3]).rolling(window=100).mean(),'g',label='lr0.0025',alpha=1)
lrs005=ax.plot(pd.Series(scores[4]).rolling(window=100).mean(),'m',label='lr0.005',alpha=1)
ax.legend(loc=0)
ax.set_title("The Effect of Learning Rate",size=20)
