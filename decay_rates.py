...
decay_rates = [0.0005, 0.0001, 0.00005,0.000005]
scores, epsilons = [0,0,0,0], [0,0,0,0]


# In[38]:


for _, decay_rate in enumerate(decay_rates):
    scores[_], epsilons[_] = train(decay_rate=decay_rate, name="decay_rate"+str(decay_rate))
    pd.Series(scores[_]).to_csv("scores_decay_rate"+str(decay_rate))
    pd.Series(epsilons[_]).to_csv("epsilons_decay_rate"+str(decay_rate))
...
decay_rate0005=ax.plot(pd.Series(scores[0]).rolling(window=100).mean(),'r',label='decay_rate0.0005',alpha=1)
decay_rate0001=ax.plot(pd.Series(scores[1]).rolling(window=100).mean(),'b',label='decay_rate0.0001',alpha=1)
decay_rate00005=ax.plot(pd.Series(scores[2]).rolling(window=100).mean(),'m',label='decay_rate0.00005',alpha=1)
decay_rate000005=ax.plot(pd.Series(scores[3]).rolling(window=100).mean(),'y',label='decay_rate0.000005',alpha=1)
ax.legend(loc=0)
...
fig=plt.figure(figsize=(12,6),dpi=200)
ax=fig.add_axes([0,0,1,1])
decay_rate0005=ax.plot(epsilons[0],'r',label='decay_rate0.0005',alpha=1)
decay_rate0001=ax.plot(epsilons[1],'b',label='decay_rate0.0001',alpha=0.9)
decay_rate00005=ax.plot(epsilons[2],'m',label='decay_rate0.00005',alpha=0.5)
decay_rate000005=ax.plot(epsilons[3],'y',label='decay_rate0.000005',alpha=0.5)
ax.legend(loc=0)
ax.set_title("Decay Rate",size=20)
ax.set_xlabel("Episodes")
ax.set_ylabel("Score")
