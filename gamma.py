...
for _, gamma in enumerate(gammas):
    scores[_], epsilons[_] = train(gamma=gamma, name="gamma"+str(gamma))
    pd.Series(scores[_]).to_csv("scores_gamma"+str(gamma))
    pd.Series(epsilons[_]).to_csv("epsilons_gamma"+str(gamma))
...
gamma999=ax.plot(scores[0],'r',label='Training Score')
ax2=ax.twinx()
epsilon=ax2.plot(epsilons[0],'b',label='Epsilon')
ax.legend(loc=0)
ax2.legend(loc=0)
ax.set_title("Lunar Lander Training",size=20)
...
fig=plt.figure(figsize=(12,6),dpi=200)
ax=fig.add_axes([0,0,1,1])
gamma999=ax.plot(pd.Series(scores[0]).rolling(window=100).mean(),'r',label='gamma0.999',alpha=1)
gamma99=ax.plot(pd.Series(scores[1]).rolling(window=100).mean(),'b',label='gamma0.99',alpha=1)
gamma9=ax.plot(pd.Series(scores[2]).rolling(window=100).mean(),'y',label='gamma0.9',alpha=1)
gamma8=ax.plot(pd.Series(scores[3]).rolling(window=100).mean(),'g',label='gamma0.8',alpha=1)
gamma7=ax.plot(pd.Series(scores[4]).rolling(window=100).mean(),'m',label='gamma0.7',alpha=1)
ax.legend(loc=0)
ax.set_title("The Effect of Gamma",size=20)
ax.set_xlabel("Episodes")
ax.set_ylabel("Score")
